...
@pytest.mark.parametrize(['env_name', 'env_cfg'], single_agent_cfgs.items())
def test_gym_api(env_name, env_cfg):
    seed = hash(env_name)
    for _ in range(NUM_SEEDS):
        seed = abs(hash(str(seed)))
        check_env(single_taxi_v0.gym_env(**env_cfg, initial_seed=seed), skip_render_check=False)


# The env checker below was taken adapted from:
...
def _is_image_space_channels_first(observation_space: spaces.Box) -> bool:
    """
